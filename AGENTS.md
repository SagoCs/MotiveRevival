...
- [ ] **Movement VIII — The Living Void (chartered + amended 2026-08-27; phases in order, full charter in README ledger).**
  ...
  - [ ] Phase 3 — Lantern cursor: full replacement (`cursor: none`); user's lantern.svg core sprite, code-drawn bloom/accent-rim/wake; states default / interactive-ring / drag / edge-chevron; native I-beam yield on inputs; flourishes gated by master motion.
  - [ ] Phase 4 — Pointer bias: pointer-driven parallax on the sky planes and horizon haze; lantern brighten/part radius.
  - [ ] Phase 5 — Usability: first-run guidance, empty states, scan/error clarity, playlist duplicate protection, accent focus-visible rings, BPM tag. Folder-picker deferred to Movement IX.
  - [ ] Phase 6 — Song-state effects: solid corner-reticle framing the playing/selected song; hovered rows get a dotted bracket + one-shot ripple (dotted = preview, solid = committed); theatrical parts gated by master motion. OPEN: whether selection marks the playing row or a keyboard-driven cursor.
  - [ ] Phase 7 — The Waveform Horizon: full-width waveform band across the timeline top (Movement VI peaks pipeline; current haze gradient as fallback until analysed), horizon-tinted and dim; lyrics render over it; musical onsets throw particles from the playhead peaks (floor/ceiling discipline); seek-on-waveform scrubber upgrade possible later.
