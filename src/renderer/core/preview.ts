...
class PreviewService {
  ...
  private async begin(track: IndexedTrack): Promise<void> {
    ...
    if (!switching && player.playing) this.duckMain();
    ...
    try {
      ...
      /* preview failed — main stays as-is */
    }
  }
  ...
  private restoreMain(instant: boolean): void {
    ...
    this.tweenMain(1, 950);
  }

  private tweenMain(to: number, ms: number): void {
    ...
    const step = (): void => {
      ...
      if (k < 1) this.fadeAnim = requestAnimationFrame(step);
      else this.fadeAnim = 0;
    };
