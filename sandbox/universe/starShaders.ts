...
uniform vec4 uSparkles[14];
uniform vec4 uSparkMeta[14];
...
void main() {
  ...
  float q1 = dPx / b1Px;
  float b1 = uBloomGain.x / pow(1.0 + q1 * q1, 3.0);
  float q2 = dPx / b2Px;
  ...
  star += bloomCol * (b1 * facet + b2 * mix(1.0, facet, 0.55) + b3) * bloomKeep;
  ...
  star += bodyCol * dBody * mix(1.0, facet, 0.25);

  float coreN = dPx / corePx;
  star += coreCol * exp(-pow(coreN, 1.9) * 2.1) * 2.0 * mix(1.0, 0.7, uFarMix);

  float lenPx = max(bodyPx * uSpikeLength, 24.0 * uDpr);
  float widPx = max(5.0 * uDpr, bodyPx * 0.1 * uSpikeWidth);
  float fuseW = 1.0 + uFuse * 1.6 * exp(-pow(dPx / (bodyPx * 1.1), 2.0));
  ...
  for (int i = 0; i < 12; i++) {
    ...
    float ny = pv.y / max(1.0, widPx * pr.z * fuseW);
    prongLight += exp(-nx * nx * 2.0) * exp(-ny * ny) * pr.w;
  }
  prongLight *= uSpikeGain;
  prongLight *= smoothstep(0.0, bodyPx * (0.5 + uFuse * 1.3), dPx);
  float prisma = clamp(dPx / (lenPx * 1.1), 0.0, 1.0) * uDisperse;
  star += mix(coreCol, uColorBloom, prisma) * prongLight;
  ...
  for (int i = 0; i < 14; i++) {
    ...
    float lobes = 0.5 + 0.5 * pow(abs(cos(4.0 * mang + meta.z * 6.2831)), 3.0);
    float starMod = mix(0.5, lobes, step(0.5, meta.w));
    ...
    vec3 scol = mix(coreCol, mix(uColorBody, uColorBloom, meta.y), 0.5 + meta.y * 0.35);
    star += scol * shape * uSparkleGain * tw;
  }
  ...
  vec2 local = fragCoord - uViewOrigin;
  float borderX = min(local.x, uResolution.x - local.x);
  float borderY = min(local.y, uResolution.y - local.y);
