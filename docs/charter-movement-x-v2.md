...
**Status:** Rechartered 2026-09-13, pending owner stamp. Supersedes `docs/charter-movement-x.md`, which stays on disk as the rejected-direction record (same disposition as the Listening View charter). **Amended 2026-09-14:** the letter ruler built early as Phase 1.5; the discography and album place redesigned away before being built — one river species serves everything (§2, §4, §5). **Amended 2026-09-15:** the artist-river entrance rebuilt as **the parting** — staged fade beats, no zoom, the blurred-art veil retired (§4). **Amended 2026-09-16:** the artist river is a hybrid by size — rivers that fill the band ring (newest centered, oldest wrapping above; an owner-ruled reversal of the ring retirement), smaller rivers are bounded in 2 / 1 / 3 order (second-newest above, newest centered, oldest last); the summon landing law partially delivered — artist, album and song results land through the shelf ceremony (§7). **Amended 2026-09-19:** the shelf’s infinite ring wakes once the field exceeds the visible band (was twice the band) — the playlists lens loops, with the + square riding the ring; **Amended 2026-09-17:** Phase 4 step 1 stamped and committed as "playlist system implementation" — the playlist river's anchor line removed by owner ruling, duplicate playlist names refused at the store, the Playlists lens un-pinned (small sets scroll bounded), bounded ends hard-stop (§6).
...
**Entrance — the parting (owner-specified choreography; supersedes part-and-zoom, amended 2026-09-15).** Two beats. **Beat one:** the squares to the left and right of the chosen artist slide away to the sides (~520ms) while the chosen square **stands** at center, fully lit, the letter ruler and lens switcher still with it. **Beat two:** the chosen square **fades away exactly where it stands** (~280ms, pure opacity — no zoom), the instruments fade with it, and the song river rises over a **solid void floor** (~420ms) rising with the beat — the river's world is its own, self-contained; nothing behind it ever shows through, summon paths included. The old part-and-zoom choreography, its 1.8× raster cap, and the blurred-art veil are retired: nothing is held — the entrance ends, and the field stays vacated behind the floor. **Esc reverses the beats in exact mirror order**: the river sinks while the floor dissolves and the artist square re-forms at center (~280ms) with the instruments returning around it — then the shelf squares slide home (~560ms). The shelf **re-centers on the artist just left** (the shelf opens where you are; owner-ruled 2026-09-14), and the field **settles silently at entry** so the return is one motion. **Reversibility is an acceptance requirement, not garnish** — the collapse is the same timeline run backwards, with no cuts and no snaps; staging means beats in mirror order, not everything at once. Text law: transits move faces, never captions — captions and ledgers fade at transit start and reappear only at rest.
...
**Amended 2026-09-15: the dim gained a second road — the shelf gestures.** On the shelf, tapping empty space (never the ruler, the lens switch, or a square): **double-tap glides the playing artist's square to center**; **triple-tap glides, enters through the full square-zoom, and lands the river scrolled exactly to the playing song**. Nothing playing, the taps are inert. The ~250ms multi-tap window is accepted. Keyboard joins too: **up/down steps one song on any river; left/right jumps album-to-album (ring-aware) on the artist river and steps artist-to-artist on the shelf; Enter activates the centered card** — enters the artist on the shelf, plays the centered song on a river.
...
**Built (step 1, 2026-09-16; stamped + committed 2026-09-17 as "playlist system implementation"):** the playlist river — playlists enter via the parting from the Playlists lens, the personal spectrum tones, two-line cards, song menu on rows, ghost entries dim + inert (contract `ghost` flag), click plays with the playlist as queue context, Esc to the Playlists lens, summon landing through the shelf ceremony (old detail layer as fallback). **Amended 2026-09-17 by owner rulings:** the anchor line (name + "N albums · M songs" under the bezel) was removed — entering a playlist is river only; duplicate playlist names are refused at the store (accent-folded, case-insensitive, rename included) with a shared shake refusal in every naming field; and bounded ends **hard-stop** ("no shaking, it just stops" — a rubber-band wall acknowledgment was built, probe-verified, and removed the same day). With every face visible in small fields, the visible end reads honestly.

**Known bugs:**
1. **No delete path from the playlist river** (open). Whole-playlist removal awaits the management scene (below), but song-level removal has no home yet either — the row menu offers no "remove from this playlist".
2. ~~Small playlist rivers refuse to scroll~~ — **FIXED 2026-09-17.** The report was the Playlists **lens**: the shelf's small-set pin froze the layout anchor mid-field and silently swallowed every scroll gesture. The shelf now rides the river's small-set grammar (1–2 pin, 3+ scroll bounded), the Shorten squash exempts sets within the band (every face stays visible at every position, shelf and rivers alike), the playlists store loads at boot, and the library's landing rebuilds both shelf fields (playlist faces had been resolving against an empty library and never repainting — the "no thumbnails" regression, fixed with it).

**Still to build:** drag-to-reorder (card-drag reorders with the insertion line, void-drag pans, wheel scrolls; one drop one persist via `reorderTrack`); the square management scene (slide-away field, Rename left / Delete right, red hover, "Certain?" two-step confirm, Esc restores); the queue river + filing landing — all three now chartered in full, with owner rulings, in `docs/charter-queue-playlist-worlds.md` (2026-09-17: the song menu slims to a two-option fork; Add to queue appends at the bottom and becomes a momentary queue river on the timeline button; Add to playlist lands the Playlists lens in a filing mode with the + square; right-click a playlist square for Rename/Delete).

**One less layer.** A playlist has no discography — it is flat: personally ordered songs scattered across albums. So there is no cascade entrance and no middle level. The songs rise directly as the vertical river over the parting, same as every world.
...
- **Phase 2 — The Artist river (spec §4.1; absorbs the former album place and discography, redesigned 2026-09-14; entrance rebuilt as the parting 2026-09-15).** Staged fade entrance (no zoom, no veil — see §4); album-grouped, album-tinted river with two-line cards; the summon album dim with bounded scroll; fully reversible collapse. Probe-verifiable: staged beats (chosen stands through beat one), river fully risen over the floor at rest, motion continuity, bounded clamp, band-tint pixels, Esc ladder.
