...
**Still to build:** drag-to-reorder (card-drag reorders with the insertion line, void-drag pans, wheel scrolls; one drop one persist via `reorderTrack`); the square management scene (slide-away field, Rename left / Delete right, red hover, "Certain?" two-step confirm, Esc restores); the queue river + filing landing (transport queue button opens a momentary queue river centered on the playing song; "Add to queue" appends and says "Added"; "Add to playlist" lands the Playlists lens, click the square → "Added" / "Already there", auto-return).
