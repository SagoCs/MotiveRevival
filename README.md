...
| VIII | The Living Void | Lantern cursor, signature constellations, parallax depth, usability block | Re-scoping — ember particles deferred |
...
- **2026-08-27 — Ember particles deferred; the browsing floor becomes a music-breathing color wash.** After repeated roadblocks the canvas ember system was removed (`player.spectrum()` remains for a future pass), and the blurred-artwork backdrop was replaced by a palette color wash — low-opacity, masked so the horizon haze keeps the floor, and gently modulated by the live bass/mid bands under the pulse flag. The archaic CSS starfield far plane (the source of the "random dots" on the timeline and elsewhere) was removed entirely. Meanwhile the "inaccurate UI colors" report was traced to hue derivation from near-neutral palette tones — a near-black dominant carries a mathematically arbitrary hue (landing on red) that the derivation's saturation floor then amplifies. Fix proposed, pending approval.
...
Movement VIII — **The Living Void** — is chartered (see ledger) and being re-scoped: canvas ember particles are deferred after repeated roadblocks, and in their place the browsing floor now carries a subtler replacement — a palette color wash that lightly breathes with the live music. The movement's remaining core: the lantern cursor, signature constellations, parallax depth, and the usability block. All motion remains gated by the master and granular flags.
