...
const r = await evalJs(`(() => {
  const probe = (sel) => {
    ...
    for (let i = 3; i < d.length; i += 4) {
      if (d[i] > 8) { lit++; sum += d[i]; if (d[i] > peak) peak = d[i]; }
    }
    return { lit, peak, avgAlpha: lit ? Math.round(sum / lit) : 0 };
  };
  return { deep: probe('.uni-deep'), near: probe('.uni-near'), tw: document.querySelectorAll('.uni-twinkle span').length };
})()`);
